...
// Ключ, под которым список задач хранится в localStorage
...
function App() {
  ...
  // Восстановление задач из localStorage при запуске.
  // Читаем в инициализаторе useState, а не в useEffect: правило
  // react-hooks/set-state-in-effect запрещает синхронный setState внутри эффекта.
  const [todos, setTodos] = useState(() => {
    ...
    try {
      ...
      console.error('Ошибка при чтении задач из localStorage:', err.message);
    }
    return [];
  });

  // Загрузка данных о курсах валют и погоде
  useEffect(() => {
    async function fetchAllData() {
      try {
        ...
        // Определяем позицию пользователя и запрашиваем погоду по координатам
  ...
  // Автосохранение задач в localStorage при каждом изменении списка
  useEffect(() => {
    try {
      localStorage.setItem(TASKS_STORAGE_KEY, JSON.stringify(todos));
    } catch (err) {
      console.error('Ошибка при сохранении задач в localStorage:', err.message);
    }
  }, [todos]);
